...
The concept of Static, Dynamic, and _Regular_ Network Objects only differs by their presence handling over the network.<br></br>
...
:::note
If the network object is registered in the <a href={pages.gnom.naming.nol}>GNOL tree</a>, `linker.findPresence` will return `Some(NetworkObjectPresence)`
...
the return value isn't likely to return `None`, it could return `Some(NetworkObjectPresence)` with the presence state for current engine set to `NOT_PRESENT`
:::
...
Static Network Objects are specifically handled by the persistence system.<br></br>
